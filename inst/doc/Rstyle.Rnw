...
as using the function called \lstinline!print.function()! to review
the contents of a function from an R package. For example, try ``lm''.
...
would be a space, as in ``\lstinline!if (x < 7)!''. However, in
R, \lstinline!if()! and \lstinline!for()! are functions, and we
do not insert spaces between function names and their parentheses.
About 90\% of the time, the R source does not include a space after
``if'' or ``for''. 
\end{enumerate}
...
Did you notice ``\lstinline!} else {!'' in the \lstinline!tidy.source()!
output for \lstinline!myfn()!? That's the correct style. We should
not have the left squiggly brace ``\lstinline!}!'' on a separate
line from the ``\lstinline!else!,'' and the right squiggly brace
``\lstinline!{!'' should be on that same line. This is, well, obviously
good (in my opinion).
...
The if's help page (run \lstinline!help("if")! or \lstinline!?"if"!)
is referring to this problem when it says, 
\begin{quote}
In particular, you should not have a newline between \textquoteleft{}\}\textquoteright{}
...
one uses squiggly braces, always follow the ``\lstinline!} else {!''
policy. 
...
``\lstinline!} else {!'' policy is ignored in vast expanses of
the R source code. In the R source code, scan for the keyword else
and in almost every file, one finds:
...
to urge code writers to use ``\lstinline!} else {!'' and make them
responsible for errors that result from ignoring that rule. 
...
that R users can follow either style, while keeping in mind the ``\lstinline!} else {!''
...
Don't write functions named ``\lstinline!rep()!'', ``\lstinline!seq()!'',
``\lstinline!c()!'', and so forth. Notice that my new function
\lstinline!lm()! does not obliterate the one from the stats package,
but it sure does make it harder to use it.
...
As long as we remember that \lstinline!lm()! is in the namespace
stats, we can find it. 
...
a new function like \lstinline!seq()! or \lstinline!lm()! into a
package if we want to. Nevertheless, almost everybody will hate to
read code like that. 
...
without the namespace name as a prefix. We might write \lstinline!base::seq(1, 10, length.out = 40)!
to be clear, but we need only write \lstinline!seq(1, 10, length.out = 40)!
...
\lstinline!graphics::par()! where, until recently, that would have
simply been \lstinline!par()!.


\subsection{(.65 SEA)Use periods to indicate classes, otherwise don't use periods
in function names. }

Instead, use camel case to name functions. This function name \lstinline!mySuperThing()!
is better than \lstinline!my.super.thing()!. 
...
of objects, such as \lstinline!print.function()! or \lstinline!print.lm()!.
...
\lstinline!my.super.thing()! suggests the user might have an object
of class ``thing'' and that \lstinline!my.super(x)! would diagnose
the class of x and send the work to \lstinline!my.super.thing()!.
A camel cased function name \lstinline!mySuperThing()! will not convey
the wrong meaning. 
...
S function names that are still used in R, like \lstinline!read.table!
and \lstinline!read.csv!. Those are not method implementations of
a generic function read(). The period is simply part of a shorthand
of the form ``action.qualifier''. Otherwise, if one had an object
of type table, then read(x) would call read.table(x). But it does
not:
...
would be named \lstinline!readTable()! and \lstinline!readCSV()!. 
...
called \lstinline!url.show()!, which follows none of the styles that
I recognize. There's no class \lstinline!show! and \lstinline!url()!
is not a generic function. In the ``action.qualifier'' tradition,
it would be \lstinline!show.url()!. And why not \lstinline!showURL()!?
I hasten to point out that the same file includes some camel cased
functions like \lstinline!defaultUserAgent()!.
...
(such as \lstinline!boxcox()!). In contrast, car calls that \lstinline!boxCox()!.
...
camel case names (\lstinline!browseVignettes()!) or underscores (\lstinline!get_all_vars()!).
...
R's own function \lstinline!rep()! with my variable. \lstinline!rep()!
